...
def crawl_price(date):
    ...
    df = df.rename(columns={'最後買價': '最後揭示買價', '最後買量(千股)': '最後揭示買量', '最後賣價': '最後揭示賣價',
                            '最後賣量(千股)': '最後揭示賣量'})
    df = df.set_index(['stock_id', 'date'])
    df = df[:-1]
...
def add_to_sql(conn, name, df):
    ...
    ret = pd.read_sql(
        'SELECT * FROM ' + name + ' INDEXED BY ix_{}_stock_id_date WHERE date BETWEEN "{}" AND "{}" '.format(name,
                                                                                                             start_date,
                                                                                                             end_date),
        conn,
    ...
    try:
        ...
        conn.execute(
            'DELETE FROM ' + name + ' INDEXED BY ix_{}_stock_id_date WHERE date BETWEEN "{}" AND "{}"'.format(name,
                                                                                                              start,
                                                                                                              end))
        conn.commit()
