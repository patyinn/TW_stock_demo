...
class HistoryRecorder(object):
    ...
    def __calculate_balance(self, date, position):
        ...
        v1 = (df[stock_list1]['證券代號'].map(lambda v: position[v]).astype(float) * df[stock_list1]['收盤價'].astype(
            float) * 1000).sum()
        v2 = (df[stock_list2]['證券名稱'].map(lambda v: position[v]).astype(float) * df[stock_list2]['收盤價'].astype(
            float) * 1000).sum()
        ret = v1 + v2
