...
def patch2019(df):
    ...
    df.columns = pd.MultiIndex(levels=[df.columns.levels[1], df.columns.levels[0]],
...
def pack_htmls(year, season, directory):
    ...
    ret = {'balance_sheet': clean(year, season, balance_sheet), 'income_sheet': clean(year, season, income_sheet),
           'income_sheet_cumulate': clean(year, season, income_sheet_cumulate),
           'cash_flows': clean(year, season, cash_flows)}
...
def fill_season4(tbs):
    ...
    for y in years:
        ...
        df4 = tbs['income_sheet_cumulate'][ds4 & ys].apply(lambda s: pd.to_numeric(s, errors='coerce')).reset_index(
            'date')
        ...
        df3 = tbs['income_sheet_cumulate'][ds3 & yps].apply(lambda s: pd.to_numeric(s, errors='coerce')).reset_index(
            'date')
...
def to_db(tbs):
    ...
    for i, df in tbs.items():
        print('  ', i)
        df = df.reset_index().sort_values(['stock_id', 'date']).drop_duplicates(['stock_id', 'date']).set_index(
            ['stock_id', 'date'])
